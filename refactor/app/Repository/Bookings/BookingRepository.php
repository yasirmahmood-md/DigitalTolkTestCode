...
namespace DTApi\Repository\Bookings;
...
use DTApi\Repository\Throttles;
use Event;
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        if ($job->due <= Carbon::now()) {
            ...
            if ($changeDue['dateChanged']) {
                $notification->sendChangedDateNotification($job, $old_time);
            }

            if ($changeTranslator['translatorChanged']) {
                $notification->sendChangedTranslatorNotification($job, $current_translator, $changeTranslator['new_translator']);
            }

            if ($langChanged) {
                $notification->sendChangedLangNotification($job, $old_lang);
            }
        }
    }
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $notification->sendSessionStartRemindNotification($user, $job, $language, $job->due, $job->duration);
            $notification->sendSessionStartRemindNotification($translator, $job, $language, $job->due, $job->duration);
            return true;
        } else {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if (isNeedToSendPush($user->id)) {
                    ...
                    $notification = app(NotificationInterface::class);
                    $notification->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, isNeedToDelayPush($user->id));
                }
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if (isNeedToSendPush($translator->id)) {
                    $users_array = array($translator);
                    $notification = app(NotificationInterface::class);
                    $notification->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if (isNeedToSendPush($customer->id)) {
                        $users_array = array($customer);
                        $notification = app(NotificationInterface::class);
                        $notification->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text, isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status = 'pending';
                $job->created_at = date('Y-m-d H:i:s');
...
//                Event::fire(new JobWasCanceled($job));
                ...
                sendNotificationTranslator::dispatch(['job' => $job, 'exclude_user_id' => $translator->id, 'data' => $data]);   // send Push all sutiable translators
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
        ...
        if (isset($affectedRows)) {
            ...
            $notification->sendNotificationByAdminCancelJob($new_jobid);
